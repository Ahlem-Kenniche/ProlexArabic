...
├── database/            # Notes for optional full-data regeneration (full database not distributed)
├── outputs/             # Expected examples and full-run quantitative statistics
├── docs/                # Reproduction, API, evaluation, and scope notes
...
The complete SQL→RDF and SQL→ProLMF pipelines use a local Prolexbase MySQL database. The full Prolexbase database is **not distributed in this repository**. The public package intentionally provides only the curated sample under `data_sample/` for executable reviewer reproduction. Authorized users who already have access to the full Prolexbase database can copy `config.example.yaml` to `config.yaml`, add their local credentials/database name, then run:
...
This EACL demonstration package is non-anonymous. Author information may therefore appear in the repository. Private credentials are **not** distributed. The included `.gitignore` excludes local secrets such as `config.yaml`, `.env`, Streamlit secrets, local feedback files, Python caches, and large/private database dumps.

## Evaluation and scope

For reviewer-facing clarification of the current runtime architecture, evaluation levels, known limitations, and the distinction between the public sample and the full processing pipeline, see [`docs/evaluation_and_scope.md`](docs/evaluation_and_scope.md).

## License

The **ProlexArabic source code** is released under the MIT License (`LICENSE`). **Prolexbase data**, including the bundled sample subset, remain governed separately by the original Prolexbase terms described in `LICENSE-PROLEXBASE`. The full Prolexbase database is not distributed in this repository.

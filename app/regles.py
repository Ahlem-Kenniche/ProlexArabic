...
def regle_P1_alif(mot):
    ...
    }, "Rule P-1 — Ending ا (alif): replace ا with the Nisba suffixes"
...
def regle_P2_ain(mot):
    ...
    }, "Rule P-2 — Ending ان: add the Nisba suffixes directly"
...
def regle_P3_ya(mot):
    ...
    return {
        ...
        "pluriel_masculin_oblique": racine + "يين",
        "pluriel_feminin": racine + "يات",
    }, "Rule P-3 — Ending يا: replace يا with the Nisba suffixes"
...
def regle_P4_compose(mot):
    ...
    explication = f"Rule P-4 — Compound name: extract the root word “{mot_racine}”, then apply {sous_regle}"
...
def appliquer_regles_ville(mot):
    ...
    if pivot:
        if pivot in LEXIQUE_EXCEPTIONS:
            return LEXIQUE_EXCEPTIONS[pivot], f"Rule V-0 — Compound city name “{mot}” → pivot “{pivot}” (lexicalized)"
        ...
        }, f"Compound city name “{mot}” → pivot “{pivot}”, then add the suffixes"
    ...
    if base.endswith("ة"):
        ...
        return {
            ...
            "pluriel_masculin_oblique": racine + "يين",
            "pluriel_feminin": racine + "يات",
        }, "Rule V-1 — Ending ة (ta marbuta): remove ة, then add the Nisba suffixes"
...
def appliquer_instances(mot, categorie):
    ...
    if categorie in ("dynastie", "ethnonyme"):
        if base.endswith("ون"):
            ...
            return {
                "situation": 2,
                "description": (
                    f"Rule I-2 — Visible alternation ✅ — The word “{base}” ends with ون\n"
                    "→ Sound masculine plural (جمع مذكر سالم) detected.\n"
                    "This is the only case where the spelling changes visibly in unvocalized Arabic text: "
                    "ون (nominative) ↔ ين (accusative/genitive)."
                ),
                "formes": {
                    "رفع (Nominative / subject)": base,
                    "نصب / جر (Accusative–Genitive / complement)": racine + "ين",
                },
            }
        if base.endswith("ين"):
            racine = base[:-2]
            return {
                "situation": 2,
                "description": (
                    f"Rule I-2 — Visible alternation ✅ — The word “{base}” ends with ين\n"
                    "→ Sound masculine plural (جمع مذكر سالم) detected.\n"
                    "This is the only case where the spelling changes visibly in unvocalized Arabic text: "
                    "ون (nominative) ↔ ين (accusative/genitive)."
                ),
                "formes": {
                    "رفع (Nominative / subject)": racine + "ون",
                    "نصب / جر (Accusative–Genitive / complement)": base,
                },
            }
    ...
    if base.endswith("ا") or base.endswith("ى"):
        ...
        return {
            "situation": 1,
            "description": (
                f"Rule I-1 — Graphically stable ✅ — The word “{mot}” ends with {term}\n"
                "→ In unvocalized Arabic writing, this ending keeps the proper name "
                "graphically unchanged across all grammatical cases."
            ),
            "formes": {
                "رفع — Nominative (subject) مرفوع": mot,
                "نصب — Accusative (complement) منصوب": mot,
                "جر — Genitive (preposition) مجرور": mot,
            },
        }
    ...
    if _mot_dans_liste(mot, _toutes_listes):
        ...
        return {
            "situation": 3,
            "description": (
                f"Rule I-1 — Known Arabic inflection\n"
                f"The word “{mot}” is listed in the database → standard Arabic case inflection applies.\n"
                "In unvocalized text, the three forms are graphically identical "
                "(they differ only in the final vowel)."
            ),
            "formes": {
                "رفع — Nominative (subject) مرفوع": mot + "\u064f",
                "نصب — Accusative (complement) منصوب": mot + "\u064e",
                "جر — Genitive (preposition) مجرور": mot + "\u0650",
            },
        }
    ...
    return {
        "situation": 3,
        "description": (
            f"Rule I-1 — Unlisted word — unchanged form\n"
            f"The word “{mot}” is not listed in the database → treated as a foreign word.\n"
            "This proper name does not take Arabic case vowels. "
            "The form remains identical in all cases."
        ),
        "formes": {
            "رفع — Nominative (subject) مرفوع": mot,
            "نصب — Accusative (complement) منصوب": mot,
...
def appliquer_regles(mot, categorie):
    ...
    mot = normaliser(mot).strip()
    if not mot:
        return _resultat_vide(), "No input provided"
    ...
    return None, (
        f"{nom_fr}: no general productive derivation rule is retained in this version. Any attested derivatives will be handled later as lexicalized forms."
    )
...
LABELS_FORMES = {
    ...
    "pluriel_masculin_nominatif": ("جمع مذكر مرفوع", "Masculine plural (nominative)"),
    "pluriel_masculin_oblique": ("جمع مذكر منصوب/مجرور", "Masculine plural (oblique)"),

...
_DEFAULT_STATE = {
    ...
    "groq_api_key": os.environ.get("GROQ_API_KEY", ""),  # never hard-code API keys
...
_REMOVED_T = {
    "fr": {
        ...
        "p1_desc":      "Browse the bundled reproducibility sample of Arabic prolexemes — instances, derivatives, and pivots.",
...
def page_accueil():
    ...
    st.markdown(
        ...
        "<strong style='color:#6366f1;'>Arabic proper names</strong> \u2014 instances, derivatives, morphology, and classification.</div>"
        "</div>",
        unsafe_allow_html=True,
    )
    ...
    with col_a:
        st.markdown(
            ...
            "Browse the bundled reproducibility sample of Arabic prolexemes — instances, derivatives, and pivots.</div>"
            "</div>",
            unsafe_allow_html=True,
        )
    ...
    with col_b:
        st.markdown(
            ...
            "<div style='color:#64748b; font-size:0.85rem; text-align:center; margin-top:0.5rem; line-height:1.7;'>"
            "Generate <strong style='color:#6366f1;'>instances</strong> and "
...
def _afficher_notoriete_recherche(mot):
    ...
    if not _row:
        st.markdown(
            ...
            f"is not listed in our ProLexBase.</div>",
            unsafe_allow_html=True,
        )
        return
...
def _afficher_notoriete_generale():
    ...
    if cfg_status:
        st.markdown(
            ...
            "You can also push everything at once:</div>",
            unsafe_allow_html=True,
        )
        if st.button("🚀 Push all data to MySQL now", key="btn_push_mysql", use_container_width=True):
...
def page_categorie():
    ...
    statut = cat.get("statut", "not retained in v1")
...
_TYPES_38 = [
    ...
    "Fête", "Histoire", "Météorologie", "Proper name",
]
...
_TYPE_DISPLAY_EN = {
    ...
    "Météorologie": "Meteorology", "Proper name": "Proper name",
}
...
_MODAL_INFO = {
    1: {
        ...
        "desc": "Bundled reproducibility sample of Arabic proper names with instances, derivatives, and multilingual pivots.",
        "items": [
...
def page_prolexbase():
    ...
    st.markdown(
        ...
        "<div style='font-size:0.82rem; color:#64748b; margin-bottom:0.75rem;'>"
        "Search and filter the bundled sample prolexemes by type."
        "</div>",
        unsafe_allow_html=True,
    )
...
def _afficher_relations_tab(num_pivot):
    ...
    if n_syn > 0:
        _section_relation(
            ...
            "Source: ProLexBase synonymy table.",
    ...
    if n_mer > 0:
        _section_relation(
            "🔗", "Meronymy",
            "Part–whole relation between places — e.g., فرنسا contains مارتينيك, or باريس is part of فرنسا. "
            "Source: ProLexBase meronymy table.",
    ...
    if n_acc > 0:
        _section_relation(
            "🌐", "Accessibility",
            "Spatial connection relation — e.g., باريس is accessible from/to فرنسا. "
            "Source: ProLexBase accessibility table.",
...
def page_prolexbase_detail():
    ...
    if pivot_info:
        st.markdown(
            ...
            f" — Type: <span style='color:#818cf8;'>{_display_type_en(pivot_info['FRA_TYPE'])}</span>"
...
# =============================================================================
# PAGE EXTRACTION DE TEXTE (Partie 3) — Groq / Groq-compatible LLM
# =============================================================================
...
def _extraire_et_classifier_llm(texte, api_key):
    ...
    prompt = (
        "You are an expert in Arabic proper names and lexicography. "
        "Extract ALL proper names from the following Arabic text, then classify each into EXACTLY "
    ...
    try:
        ...
        for nom, raw_type in parsed.items():
            ...
            result[nom] = normalized if normalized else "Proper name"
        return result, None
...
def page_extraction():
    ...
    1. Enter Arabic text
    2. A Groq-compatible LLM extracts candidate proper names
    3. The same LLM maps each candidate to a ProLexBase semantic type
    4. The user can review/edit the type before applying deterministic morphology rules
    """
    st.components.v1.html("<script>window.parent.scrollTo(0,0);</script>", height=0)
    ...
    with st.expander(
        "🔑 Groq API Key" + (" ✅" if st.session_state.groq_api_key else " — required for extraction & classification"),
        ...
        st.markdown(
            ...
            "console.groq.com</a> · Model: <code>configured via GROQ_MODEL</code></div>",
            unsafe_allow_html=True,
        )
    ...
    with st.spinner("🤖 Running extraction & classification with Groq-compatible LLM (Groq)…"):
    ...
    st.markdown(
        ...
        f"Extraction & Classification : <code style='color:#818cf8;'>configured Groq model</code>"
        f"</div>",
        unsafe_allow_html=True,
    )
    ...
    for nom in noms_propres:
        current_type = st.session_state.extr_classified.get(nom, "Proper name")
...
def page_base_classify():
    ...
    with col_btn:
        if st.button(
            ...
            with st.spinner("🤖 Running classification with Groq-compatible LLM (Groq)…"):
                classified, llm_err = _extraire_et_classifier_llm(
                    ...
                    if not matched:
                        ...
                        for k, v in classified_lower.items():
                            if nom in k or k in nom:
                                matched = v
                                break
                    st.session_state.base_classified[nom] = matched or "Proper name"
                st.session_state.base_classify_version += 1
                st.rerun()
    ...
    if st.session_state.base_classified:
        ...
        st.markdown(
            ...
            f"<code style='color:#818cf8;'>configured Groq model</code></div>",
            unsafe_allow_html=True,
        )
        ...
        for i, nom in enumerate(noms):
            current_type = st.session_state.base_classified.get(nom, "Proper name")
            c1, c2, c3 = st.columns([3, 3, 1.8])
...
def page_about():
    ...
    st.markdown("## About ProlexArabic")
    st.markdown(
    ...
    with c1:
        ...
        st.markdown(
            ...
            "| ⚙️ **Part 2** | Morphological rule system |\n"
            "| 📝 **Part 3** | Text extraction and classification |"
        )
    with c2:
        st.markdown("### 🎓 Academic Context")
        st.markdown(
            "Project developed as part of a Computer Science research project in Natural Language Processing (NLP).\n\n"
            "**Year** : 2025 – 2026"
        )

...
def test_mysql_connection():
    ...
    if not cfg:
        return False, "Aucune configuration MySQL enregistrée."
    try:
        import pymysql
        cnx = pymysql.connect(
...
def push_prolexeme_to_mysql(lang_code, num_prolexeme, max_retries=3):
    ...
    cfg = get_mysql_config()
    if not cfg:
        return False, "Aucune configuration MySQL."
    ...
    if not rows:
        return False, f"Aucune donnée locale pour {lang_code}/{num_prolexeme}."
    last_err = None
...
def push_all_to_mysql():
    ...
    cfg = get_mysql_config()
    if not cfg:
        return 0, "Aucune configuration MySQL."
    try:
        import pymysql
        cnx = pymysql.connect(
            host=cfg["host"], port=int(cfg["port"]),
